...
export default function FormButton(props: FormButtonProps){
    const bgcolor = () => {
        if(!props.disabled) {
            if(props.bgcolor==='cyan'){
                ...
                return 'bg-transparent border-2 border-white hover:bg-gray-200 hover:border-white hover:text-gray-800'
            }
        } else {
    ...
    return (
        <button onClick={props.onClick} className={
            `flex flex-shrink-0 justify-center font-bold rounded-lg p-2 shadow-lg min-w-fit min-h-fit py-2 cursor
            ${bgcolor()}
            ${props.className}`}
